...
module.exports = {
            ...
            if(err){
                ...
                if(userInfo){
                    if(bcrypt.compareSync(req.body.password, userInfo.password)){
                        ...
                        res.json({ code: 404, status: 'failed',
    ...
    logout : async function(req,res,next){
        if(req.cookies.token){
            ...
            res.json({code: 200, status: 'success', message: 'Logged Out..', data: null});
        }
        else{
            res.json({code: 200, status: 'error', message: 'Log in first..', data: null});
        }
    }
}